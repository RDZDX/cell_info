...
void timer(int a){
	...
	vm_cell_info_struct** bs = vm_cell_get_nbr_cell_info(); //gauti kaimyniniu stociu informacija
        ...
        sprintf(k, "ARFCN:%d, BSIC:%d, Rx signal lev:%d, MCC:%d, MNC:%d, LAC:%d, CI:%d\n", info->arfcn, info->bsic, info->rxlev-110, info->mcc, info->mnc, info->lac, info->ci);
	...
	for(i = 0; i<n; ++i) {
           ...
           sprintf(k, "ARFCN:%d, BSIC:%d, Rx signal lev:%d, MCC:%d, MNC:%d, LAC:%d, CI:%d\n", bs[i]->arfcn, bs[i]->bsic, bs[i]->rxlev-110, bs[i]->mcc, bs[i]->mnc, bs[i]->lac, bs[i]->ci);
           strcat(px, k);
