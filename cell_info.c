	...
	if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_LEFT_SOFTKEY) {
           if (flightMode == VM_FALSE) {
              ...
              if (c_y == max_lines - 1 && swich == VM_FALSE) {
                 cprintf("Data written to %s", px);
                 swich = VM_TRUE;
              } else if(c_y < max_lines - 1) {
                 cprintf("Data written to %s\n", px);
              } else {
                cprintf("\nData written to %s", px);
              }
...
void timer(int a){
	...
	vm_cell_info_struct** bs = vm_cell_get_nbr_cell_info(); //gauti kaimyniniu stociu informacija
        ...
        if(c_y == max_lines - 1 && swich == VM_FALSE) { // 0 <= 29-1 ??
          ...
          sprintf(p, "%04d-%02d-%02d %02d:%02d:%02d %d\n", curr_time.year, curr_time.mon, curr_time.day, curr_time.hour, curr_time.min, curr_time.sec, max_lines);
        } else {
        ...
        //sprintf(x, "ARFCN:%d, BSIC:%d, Received signal level:%d, MCC:%d, MNC:%d, LAC:%d, CI:%d", info->arfcn, info->bsic, info->rxlev, info->mcc, info->mnc, info->lac, info->ci);
