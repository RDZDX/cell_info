...
void console_put_char(char c) {
    if (c == '\n') {
        ...
        if ((c_y + 1) * char_height > scr_height) {
        //if ((c_y + 0) * char_height > scr_height) { // (c_y + 1)
            scroll_up();
            c_y = (scr_height / char_height) - 1;
            //c_y = (scr_height / char_height) - 0; // - 1
        }
        // Nebekviečiam flush_layer() čia, nes tai daro console_put_str()
        return;
    }
...
void console_put_str(const char* str) {
    ...
    // <-- Papildomai: atnaujiname ekraną po paskutinio simbolio
    flush_layer();
}
